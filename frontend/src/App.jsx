...
function App() {
  ...
  const handlePayout = async (e) => {
    ...
    try {
      // Convert decimal INR (e.g. 10.50) to integer Paise (1050)
  ...
  if (!merchant) return <div className="p-8 text-center font-sans">Connecting to live backend...</div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <header className="flex justify-between items-center pb-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500">Real-time Cloud Ledger (Production Mode)</p>
          </div>

          <div className="flex items-center gap-4 bg-white p-2 rounded-lg border border-gray-200 shadow-sm">
            <label className="text-xs font-semibold text-gray-400 uppercase">Active Merchant</label>
            <select
              className="rounded border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 p-1 bg-transparent font-medium cursor-pointer"
              value={merchant.id}
              onChange={(e) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Balance Cards - Formatting Paise to INR */}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col">
            <span className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Processing (Held)</span>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2 pl-7"
                    placeholder="0.00"
                  />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Target Bank Account</label>
                <input
                  type="text"
                  required
                  value={bankAccount}
            ...
            <div className="overflow-x-auto max-h-[300px] overflow-y-auto">
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-gray-100">
                  {payouts.length === 0 ? (
                    <tr><td colSpan="4" className="px-6 py-8 text-center text-gray-500 italic">No payouts found for this merchant.</td></tr>
                  ) : payouts.map(p => (
